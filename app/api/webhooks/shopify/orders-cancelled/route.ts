...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await syncCancelledOrderToAirtable(payload);

    return NextResponse.json({ success: true, ...result });
  } catch (error) {
    console.error("Shopify orders/cancelled webhook error:", error);

    return NextResponse.json(
      {
        error: "Failed to sync Shopify cancelled order.",
