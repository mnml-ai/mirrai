...
export async function POST(request: Request) {
  console.log("WEBHOOK HIT refunds-create");
  ...
  try {
    ...
    const result = await syncRefundToAirtable(payload);

    return NextResponse.json({ success: true, ...result });
  } catch (error) {
    console.error("Shopify refunds/create webhook error:", error);

    return NextResponse.json(
      {
        error: "Failed to sync Shopify refund.",
